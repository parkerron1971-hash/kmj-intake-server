...
| File | What | Status |
|---|---|---|
| `supabase/APPLY-2026-09-26-chief-build-lanes.sql` | Chief's background jobs run side by side when they touch different things: `chief_build_claim` serializes per business AND lane (`chief_build_lane`: site / image / plan) instead of per business. Same signature; replayable. | **APPLIED 2026-09-26 ~19:05 UTC** via the SQL editor, at Kevin's word ("add to supabase"). Verified: `chief_build_lane` returns `image / plan / site` for flyer / plan / event_setup; `chief_build_claim` uses lanes (`pg_get_functiondef`); `authenticated` cannot execute the claim and `anon` cannot execute the lane function; `service_role` can. Through PostgREST: the service-role claim of a missing job returns `200 []`, and an anon call returns `401 permission denied`. CI check: `scripts/chief-build-lanes-db-check.mjs`. |
