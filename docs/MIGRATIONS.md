...
| File | What | Status |
|---|---|---|
| `supabase/APPLY-2026-09-26-calendar-feeds.sql` | `calendar_feeds` (a practitioner's private iCal links: Google secret address, Outlook published calendar, iCloud, Acuity; the link is a secret, never returned whole or logged) and `calendar_busy_blocks` (busy times only: start, end, all-day, UID hash; no titles or attendees). Service-role only: RLS on, no policies, no anon/authenticated grants; both cascade with the business. Code is fail-soft without it: slots unchanged, the app card shows "coming soon". | **PENDING. Apply after the backend PR merges.** Verify with the queries at the foot of the file (RLS true on both, 0 policies, anon/authenticated SELECT false). |
