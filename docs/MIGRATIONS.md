...
| File | What | Status |
|---|---|---|
| `supabase/APPLY-2026-09-12-chief-computer-runtime.sql` | Service-only atomic plan, approval/job insertion, state/hold transitions, ordered events and settings merge. | **PENDING. Apply after PR3 merge**, after the Chief computer foundation. Requires existing `chief_jobs` and its one-active-per-business/kind index. Keep `ERRANDS_ENABLED=off` until PR4 and the independent vault key are live. |
