...
| `supabase/APPLY-2026-08-31-client-actor-and-identity.sql` | The two-sided client layer's irreversible doors: `audit_log.actor_type` gains `'client'` (the CHECK went from 4 values to 5), and `business_customers` gains a nullable `platform_identity_id` + partial index. | **PENDING — apply after merge.** Both are cheap now and unrecoverable later. `audit_log` is append-only for real (BEFORE UPDATE/DELETE triggers raise, even for service_role), so rows written under the 4-value CHECK can never be relabelled — a client actor decided after the first client action is decided too late, permanently. `platform_identity_id` is written by nothing and read by nothing today; it exists so cross-tenant client identity is still possible later, and it is explicitly NOT a cross-tenant email match. Verify: `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname='audit_log_actor_type_check';` (expect 5 values) and `SELECT to_regclass('public.business_customers') IS NOT NULL;` plus a column check. |
