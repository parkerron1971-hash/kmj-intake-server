...
| File | What | Status |
|---|---|---|
| `supabase/APPLY-2026-09-26-dev-desk-desktop-sessions.sql` | `dev_tasks.origin` (`dev_desk` default, or `desktop`; CHECK) so a session Kevin opens by hand in Solution Space can announce itself to the Dev Desk; `dev_tasks.device_id` (FK to `dev_bridge_devices`, `on delete set null`) so a device's sweep closes only its own sessions; a partial index on desktop rows. Desktop rows carry no `authority_record`, so the immutability trigger never engages. | **PENDING — apply any time; before merge is best.** Additive and idempotent; old code never reads the columns. Without it the Dev Desk still opens exactly as before (`dev_desk()` falls back to the pre-origin read), and announcing a desktop session fails loud naming this file. Verify: `SELECT column_name FROM information_schema.columns WHERE table_name='dev_tasks' AND column_name IN ('origin','device_id');` (expect 2 rows) and `SELECT count(*) FROM dev_tasks WHERE origin <> 'dev_desk';` (expect 0 right after applying). |
...
| `supabase/APPLY-2026-09-18-chief-builds.sql` | Durable build identities, fenced worker leases, versioned approval/resume, owner-scoped image reservation. | **PENDING. Apply after merge, before enabling CHIEF_BUILDS.** Requires chief_jobs and the 2026-09-08 image studio migration. Local PostgreSQL concurrency, replay, ownership and role checks passed. |
