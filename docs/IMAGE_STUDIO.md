...
3. Set `OPENAI_IMAGE_MODEL` to an explicitly supported model: `gpt-image-2.5-sunburst` (recommended and unset default), `gpt-image-2.5-flare`, or `gpt-image-2`. On 2026-09-09 the production key successfully generated images with both 2.5 models using Chief's existing headers, although `GET /v1/models/{model}` returned 404 for both. The catalog is therefore not an access gate: availability is determined by the actual Images generation/edit response. There is no automatic model substitution or paid retry.
...
Those token rates apply to GPT Image 2 and both 2.5 models; equal token rates do not guarantee equal cost per image. The GPT Image 2 calculator does not estimate 2.5 token consumption. Provider costs are recorded even if saving a generated image fails, but customer image credits are charged only after the saved artwork reaches `ready`. A provider denial does not charge image credits. Successful provider request IDs are logged for support diagnostics.

Chief's ordinary chat uses Claude Sonnet 5 unless explicitly overridden. Its standard API rates remain $2/million input tokens and $10/million output tokens; Anthropic cancelled the scheduled September 1 increase. The cost logger uses the corrected rate for new calls, including the existing cache discounts. Historical usage rows and customer credit prices are unchanged. Source: [Claude pricing](https://platform.claude.com/docs/en/about-claude/pricing), verified 2026-09-09.
