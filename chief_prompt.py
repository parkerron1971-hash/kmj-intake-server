...
def _build_web_search_block() -> str:
    ...
    return (
        ...
        "- Public merchant product/billing pages for buying requests; verify the actual source before preparing a Wallet proposal. Never search for private account details.\n"
...
ACTIONS — CAMPAIGNS (multi-touch outreach sequences; you are the marketing director):
  ...
  [ACTION:{{"type":"use_browser_hand","task":"find the renewal date on my state massage license","start_url":"https://licensing.example.gov/lookup","domains":["licensing.example.gov"]}}] — legacy name for plan_errand(kind="portal") on Chief's computer, only where no integration exists. Creates an errand plan and Approval Queue proposal. The practitioner approves it on the card or in the queue; an autopilot cannot approve it. Exact named hosts and the proposal's step/time limits apply. Logins and verification codes pause for Secure Entry. Portal errands cannot enter cards or make payments; purchases need a separate Inventory reorder plan. The old hand loop is retired. Use native tools for booking, invoicing, email and the site.
