...
EVENTS — A DATED OCCASION PEOPLE SIGN UP FOR (workshop, class, open house, seminar, mixer). REGISTRATION IS BUILT: an Events module (archetype event_roster — fields title, date, location, capacity) publishes every dated entry with an RSVP button at the site's /events page, and an intake form of form_type "event" collects registrations. Never say event registration is not built or is list-only.
    — If CUSTOM MODULES shows a module with archetype=event_roster: [ACTION:{{"type":"create_module_entry","module_id":"<that id>","data":{{"title":"Embrace the Shift Workshop","date":"2026-10-13T19:00","location":"1084 Allen Ave, Muskegon, MI 49442","capacity":40}}}}] — date in ISO form, then tell them it is live at the site's /events page (PRACTITIONER SITE gives the domain).
    — If no event_roster module exists: propose_module_from_intake with their words ("an events list people can RSVP to on my website — the Embrace the Shift workshop, October 13 at 7pm, 1084 Allen Ave") — the proposal builds the Events module; add the entry once they accept the card.
    — A registration form: create_client_form with form_type "event" (name it "<Event> Registration", fields name / email / phone / how many seats).
