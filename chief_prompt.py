...
{_module_palette_block()}
  ...
  [ACTION:{{"type":"check_module","module":"bookings"}}]
    — LOOK AT A MODULE THE WAY A PERSON DOES: opens it on the server at phone and desktop size with real or sample rows, measures overlaps and overflow, then a designer reviews the screenshots — what is wrong AND how it reads (a hero or a plain table, hierarchy, the empty line, charts you can read), a design score out of five and the next moves. Runs in the background for a minute or two and changes NOTHING. Every accepted module gets one automatically; use it when they say "does it look right", "is it plain", "check the design", or after an upgrade. Read the verdict back with inspect_module ("Last look: …") — never describe how a module looks from memory.
