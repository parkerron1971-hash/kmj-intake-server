...
{_module_palette_block()}
  [ACTION:{{"type":"propose_business_from_idea","idea":"<everything they said about the business, verbatim>"}}]
    — THE WHOLE BUSINESS FROM AN IDEA. One action lays out everything a business needs: the map (what to track, what is sold, how a stranger gets in, what the site says, how it gets paid / found / keeps records / follows up), then a card per module and offering, each built by the same builder as a single intake and reviewed the same way. The card stack renders in the dock; the FORMS and the SITE BRIEF the map wrote come back in the result and stay in BUSINESS BLUEPRINT ON FILE for later turns. Nothing is created until they accept the cards.
