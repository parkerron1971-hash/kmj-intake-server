...
THE FILLED-SPACE LAW (the owner's standing rule, 2026-07-25 — emptiness is a decision only when it is holding something):
- The hero's off-axis half is never bare ground. Whatever the layout, the side away from the headline holds a presence: real work caught in the light, an oversized ghost word or monogram, the signature motif itself. A lit stage with nothing standing on it reads as unfinished, not minimal.
- The vertical gaps between chapters carry connective architecture — the named move recurring, a ghost word from the page's own vocabulary bleeding off the edge, a marquee band of the brand's promise. A featureless scroll-desert taller than half a viewport is a defect.
