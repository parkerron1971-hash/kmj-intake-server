...
class Handler(BaseHTTPRequestHandler):
 def do_GET(self):
  if urlsplit(self.path).path=='/embed':
   ...
   page=render_form_page(BIZ,FORM,site=current_site,submit_url='/intake/submit',canonical_url='https://kmj-creative-solutions.mysolutionist.app/public/widget/form/preview',embedded=parse_qs(urlsplit(self.path).query).get('embed')==['1'])
