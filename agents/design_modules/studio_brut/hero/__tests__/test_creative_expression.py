...
_RE_H1_REM = re.compile(r"--hero-h1-size-rem:\s*([0-9.]+)")
_RE_H2_REM = re.compile(r"--hero-h2-size-rem:\s*([0-9.]+)")
_RE_WEIGHT = re.compile(r"--hero-display-weight:\s*(\d+)")


def _parse_rem_values(html: str) -> Tuple[float, float, int]:
    """Extract --hero-h1-size-rem, --hero-h2-size-rem, --hero-display-weight
    from a rendered variant's inline style. Raises if missing — every
    Pass 4.0i variant MUST emit all three."""
    m_h1 = _RE_H1_REM.search(html)
    m_h2 = _RE_H2_REM.search(html)
    m_w = _RE_WEIGHT.search(html)
    if not m_h1:
        raise AssertionError("--hero-h1-size-rem missing from rendered HTML")
    if not m_h2:
        raise AssertionError("--hero-h2-size-rem missing from rendered HTML")
    if not m_w:
...
class IntensityRubricMatrixTests(unittest.TestCase):
        ...
        for vid in variant_ids:
            for intensity in intensities:
                ...
                try:
                    ...
                    h1_rem, h2_rem, weight = _parse_rem_values(html)
    ...
    def test_variant_authors_picked_bases_above_floor(self):
        ...
        for vid in VARIANT_REGISTRY.keys():
            ...
            h1_rem, h2_rem, _ = _parse_rem_values(html)
...
class IntensityProgressionTests(unittest.TestCase):
    ...
    def test_bold_larger_than_restrained_for_mid_variant(self):
        ...
        h1_r, _, _ = _parse_rem_values(_render("edge_bleed_portrait", "restrained"))
        h1_c, _, _ = _parse_rem_values(_render("edge_bleed_portrait", "confident"))
        h1_b, _, _ = _parse_rem_values(_render("edge_bleed_portrait", "bold"))
    ...
    def test_bold_caps_at_ceiling_for_high_base_variant(self):
        ...
        h1_b, _, _ = _parse_rem_values(_render("oversize_statement", "bold"))
