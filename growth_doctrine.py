...
# ...and the same reasoning applied to modes that do not exist yet.
#
# This list named only `strategy_coach`, and by the time it shipped
# `business_coach` also existed — so the second coach was NOT excluded.
# It never leaked, because that mode returns its own prompt before
# growth_block is consumed. Which is exactly the sentence above,
# repeated: the gate that could not be reached.
#
# A coach is a distinct persona by construction, so the suffix is the
# actual rule and the list is just the exceptions to it. That way a
# third coach is excluded on the day it is named, not on the day
# somebody remembers this file.
_COACH_SUFFIX = "_coach"


def _is_excluded_mode(mode: Optional[str]) -> bool:
    m = (mode or "").strip()
    return m in _EXCLUDED_MODES or m.endswith(_COACH_SUFFIX)
