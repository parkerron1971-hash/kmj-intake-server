...
5. Images ONLY as <img data-slot="…" src="" alt="specific, evocative alt"> with data-slot from: {slots_line}. Each slot at most once. The platform fills src. THE LAYER LAW (image-overlay zones): a background photo sits on its OWN layer marked data-sx-bg="true" data-sx-zone="image-overlay" with any design treatment (gradients, tints, scrims) as SEPARATE sibling layers — swapping the photo must never remove your design. Content images carry data-sx-zone="image". Every img also carries data-sx-label="<what belongs here>" (e.g. "Founder portrait").
