...
"""
    ...
    if about_config.get("enabled", True):
        ...
        if about_text:
            about_html = f"""
<section class="cel-section cel-about reveal">
...
"""
    ...
    if services_config.get("enabled", True) and products:
        ...
        for p in products[:6]:
            ...
            cards.append(f'<div class="cel-service-card"><h3>{name}</h3>{desc_html}{price_html}{cta_html}</div>')
        services_html = f"""
<section class="cel-section reveal">
  <h2>Join us</h2>
